function p=wfg9(p,odim,k,l)
        p.name='WFG9';
        p.od=odim;
        p.pd=k+l;
        if odim==2
            p.pf=load('WFG9.2D.pf');
        else
            p.pf=load('WFG9.3D.pf');
        end
        ...
        %    z=z';
         cc=p.domain(:,:)';
            ...
            for i =1:(p.pd-1)
                t1(i) = b_param(y(i),r_sum(y((i+1):p.pd),ones(p.pd-i,1)),0.98/49.98,0.02,50);
            end
            t1(p.pd) = y(p.pd);
            for i=1:k
            ...
            t3(odim) = r_nonsep(t2((k+1):p.pd),p.pd-k);           
            x = zeros(odim,1);
            for i = 1:(odim-1)
